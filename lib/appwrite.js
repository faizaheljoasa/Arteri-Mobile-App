...
export const config = {
  ...
  dataCollectionId: '66643332003a968401b1',
  noteCollectionId: '6687c7d4001ea50c10a7',
}
...
export const getAllData = async () => {
  try {
    ...
    const allData = await databases.listDocuments(
      config.databaseId,
      config.dataCollectionId,
      [Query.equal('accountId', userDocId)],
      100, 
      0,  
      'date',
      'DESC',
...
export const getAllNote = async () => {
  try {
    ...
    const allNote = await databases.listDocuments(
      config.databaseId,
...
export const updateHasilTest = async (hasilTest) => {
  try {
    const currentAccount = await account.get();
    if (!currentAccount) throw new Error('No current account found');
    ...
    const userSettings = await databases.listDocuments(
      config.databaseId,
      config.examinationCollectionId,
      [Query.equal('examinationPatient', userDocId)]
    );
